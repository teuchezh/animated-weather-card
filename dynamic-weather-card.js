...
`,Tt={wind:I`
  ...
  `};function Et(t,...e){const i=Tt[t];return"function"==typeof i?i(...e):i||""}class Pt extends ot{static get properties(){return{hass:{type:Object},config:{type:Object}}}static get styles(){return St}constructor(){super(),this.config={},this.animationFrame=null,this.canvas=null,this.ctx=null,this.canvasWidth=0,this.canvasHeight=0,this.animations={},this.holdTimer=null,this.holdDelay=500}connectedCallback(){super.connectedCallback(),this.updateComplete.then(()=>{setTimeout(()=>{this.setupCanvas(),this.canvas&&this.ctx&&(this.initializeAnimations(),this.startAnimation(),this.setupResizeObserver()),this.setupForecastScroll()},100)})}disconnectedCallback(){if(super.disconnectedCallback(),this.animationFrame&&(cancelAnimationFrame(this.animationFrame),this.animationFrame=null),this.resizeObserver&&(this.resizeObserver.disconnect(),this.resizeObserver=null),this._wheelHandler&&this.shadowRoot){const t=this.shadowRoot.querySelector(".forecast-scroll");t&&t.removeEventListener("wheel",this._wheelHandler),this._wheelHandler=null}}updated(t){super.updated(t),(t.has("hass")||t.has("config"))&&(this.canvas&&this.ctx&&this.resizeCanvas(),this.setupForecastScroll())}initializeAnimations(){this.animations={sunny:new xt(this.ctx),rainy:new kt(this.ctx),snowy:new $t(this.ctx),cloudy:new _t(this.ctx),foggy:new Ct(this.ctx),hail:new Mt(this.ctx),thunderstorm:new At(this.ctx)}}setupResizeObserver(){if(!this.shadowRoot)return;const t=this.shadowRoot.querySelector(".canvas-container");t&&(this.resizeObserver=new ResizeObserver(()=>{this.resizeCanvas()}),this.resizeObserver.observe(t))}setupForecastScroll(){if(!this.shadowRoot)return;const t=this.shadowRoot.querySelector(".forecast-scroll");t&&(this._wheelHandler&&t.removeEventListener("wheel",this._wheelHandler),this._wheelHandler=e=>{0!==e.deltaY&&(e.preventDefault(),t.scrollLeft+=e.deltaY)},t.addEventListener("wheel",this._wheelHandler,{passive:!1}))}resizeCanvas(){if(!this.canvas)return;const t=this.shadowRoot.querySelector(".canvas-container");if(!t)return;const e=t.getBoundingClientRect();if(0===e.width||0===e.height)return;const i=window.devicePixelRatio||2;this.canvas.width=e.width*i,this.canvas.height=e.height*i,this.canvas.style.width="100%",this.canvas.style.height="100%",this.ctx=this.canvas.getContext("2d"),this.ctx.scale(i,i),this.canvasWidth=e.width,this.canvasHeight=e.height,this.initializeAnimations()}setupCanvas(){const t=this.shadowRoot.querySelector(".canvas-container");if(!t)return;const e=t.querySelector("canvas");e&&e.remove(),this.canvas=document.createElement("canvas"),t.appendChild(this.canvas),this.resizeCanvas()}getState(t){if(!this.hass||!t)return null;const e=this.hass.states[t];return e?e.state:null}getAttributes(t){if(!this.hass||!t)return{};const e=this.hass.states[t];return e?e.attributes:{}}getWeatherData(){const t=this.config.entity||"weather.home",e=this.getState(t),i=this.getAttributes(t);return{condition:i.condition||e||"sunny",temperature:i.temperature||20,apparentTemperature:i.apparent_temperature||null,humidity:null!=i.humidity?i.humidity:null,windSpeed:null!=i.wind_speed?i.wind_speed:null,windGust:i.wind_gust_speed||i.wind_gust||null,windBearing:null!=i.wind_bearing?i.wind_bearing:null,windDirection:i.wind_direction||null,pressure:i.pressure||null,forecast:i.forecast||i.forecast_hourly||[],friendlyName:i.friendly_name||"Weather",templow:i.templow||(i.forecast&&i.forecast[0]?i.forecast[0].templow:null)||(i.forecast_hourly&&i.forecast_hourly[0]?i.forecast_hourly[0].native_templow:null)}}getTodayForecast(){if(!this.hass||!this.config)return[];const t=this.getWeatherData();if(!t.forecast||0===t.forecast.length)return[];const e=new Date,i=new Date(e.getFullYear(),e.getMonth(),e.getDate()),s=new Date(i);s.setDate(s.getDate()+1);return t.forecast.filter(t=>{if(!t.datetime)return!1;const n=new Date(t.datetime),a=new Date(n.getFullYear(),n.getMonth(),n.getDate());return a.getTime()===i.getTime()||a.getTime()===s.getTime()&&n.getHours()<=e.getHours()}).sort((t,e)=>new Date(t.datetime)-new Date(e.datetime)).slice(0,8)}startAnimation(){const t=()=>{this.draw(),this.animationFrame=requestAnimationFrame(t)};t()}draw(){if(!this.ctx||!this.canvas)return;if(!(this.canvasWidth&&this.canvasHeight||(this.resizeCanvas(),this.canvasWidth&&this.canvasHeight)))return;const t=this.canvasWidth,e=this.canvasHeight;this.ctx.clearRect(0,0,t,e);const i=this.getWeatherData(),s=this.hass?.states[this.config.entity],n=vt(s),a=this._testTimeOfDay||yt(n);switch(i.condition.toLowerCase()){case"sunny":case"clear":this.animations.sunny.draw(a,t,e);break;case"clear-night":this.animations.sunny.draw({type:"night",progress:0},t,e);break;case"rainy":case"rain":this.animations.rainy.draw(a,t,e,!1);break;case"pouring":this.animations.rainy.draw(a,t,e,!0);break;case"snowy":case"snow":this.animations.snowy.draw(a,t,e);break;case"snowy-rainy":this.animations.rainy.draw(a,t,e,!1),this.animations.snowy.draw(a,t,e);break;case"hail":this.animations.hail.draw(a,t,e);break;case"foggy":case"fog":this.animations.foggy.draw(a,t,e);break;case"lightning":this.animations.thunderstorm.draw(a,t,e,!1);break;case"lightning-rainy":this.animations.thunderstorm.draw(a,t,e,!0);break;default:this.animations.cloudy.draw(a,t,e)}}renderTodayForecast(){const t=this.getTodayForecast();return 0===t.length?L`<div style="opacity: 0.6; font-size: 14px;">Прогноз недоступен</div>`:L`
      <div class="forecast-scroll">
        ${t.map(t=>{return L`
          <div class="forecast-item">
            <div class="forecast-time">${i=t.datetime,i?`${new Date(i).getHours().toString().padStart(2,"0")}:00`:""}</div>
            <div class="forecast-icon">${e=t.condition,e&&Nt[e.toLowerCase()]||""}</div>
            <div class="forecast-temp">${Math.round(t.temperature||t.temp||t.native_temperature||0)}°</div>
          </div>
        `;var e,i})}
      </div>
    `}getLanguage(){if(this.config.language&&"auto"!==this.config.language)return this.config.language;if(this.hass&&this.hass.language){return"ru"===this.hass.language.split("-")[0]?"ru":"en"}return"en"}getConditionName(t){const e=this.getLanguage();return(ht[e]||ht.en)[t.toLowerCase()]||t}translate(t){const e=this.getLanguage();return(dt[e]||dt.en)[t]||t}render(){if(!this.hass)return L`<div>No Home Assistant connection</div>`;const t=this.getWeatherData(),e=vt(this.hass.states[this.config.entity],this.config.sunriseEntity,this.config.sunsetEntity,this.hass),i=this._testTimeOfDay||yt(e),s=`weather-card ${i.type}`;let n=this.config.height?`${this.config.height}px`:"200px";const a=function(t){if("sunrise"===t.type){const e=t.progress,i={r:26,g:26,b:46},s={r:255,g:160,b:122},n={r:255,g:215,b:0};return{start:{r:Math.round(i.r+(s.r-i.r)*e),g:Math.round(i.g+(s.g-i.g)*e),b:Math.round(i.b+(s.b-i.b)*e)},end:{r:Math.round(i.r+(n.r-i.r)*e),g:Math.round(i.g+(n.g-i.g)*e),b:Math.round(i.b+(n.b-i.b)*e)}}}if("sunset"===t.type){const e=t.progress,i={r:255,g:107,b:107},s={r:255,g:160,b:122},n={r:26,g:26,b:46};return{start:{r:Math.round(i.r+(n.r-i.r)*e),g:Math.round(i.g+(n.g-i.g)*e),b:Math.round(i.b+(n.b-i.b)*e)},end:{r:Math.round(s.r+(n.r-s.r)*e),g:Math.round(s.g+(n.g-s.g)*e),b:Math.round(s.b+(n.b-s.b)*e)}}}return null}(i),r=a?`background: linear-gradient(135deg, rgb(${a.start.r}, ${a.start.g}, ${a.start.b}), rgb(${a.end.r}, ${a.end.g}, ${a.end.b}));`:"";return L`
      <ha-card
      ...
      >
        <div class="${s}" style="min-height: ${n}; ${r}; cursor: pointer;">
          ...
          <div class="content">
            ...
            <div class="details">
              <div class="info-grid">
                ...
                ${this.config.showSunriseSunset&&e.hasSunData?L`
                  <div class="info-item">
                    <span class="info-icon">${Et("sunset")}</span>
                    <span>${wt(e.sunset)}</span>
                  </div>
                `:""}
              </div>
            </div>
